...
int Command_build(apr_pool_t *p, const char *url, const char *configure_opts, const char *make_opts, const char *install_opts) {
  ...
  if (access(AUTO_GEN_SCRIPT, X_OK) == 0) {
    log_info("Has autogen script, running that.");
    rc = Shell_exec(AUTO_GEN_SH, NULL);
    check(rc == 0, "Failed to autogen.sh");
  }

  if (access(AUTO_CONFIGURE_SCRIPT, X_OK) == 0) {
    log_info("Has a auto configure script, running autoreconf");
    rc = Shell_exec(AUTO_CONFIGURE_SH, NULL);
    check(rc == 0, "Failed to auto reconfigure.");
  }
