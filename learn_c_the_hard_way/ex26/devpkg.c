...
int main(int argc, const char *argv[]) {
  ...
  while (apr_getopt(opt, "I:Lc:m:i:d:SF:B:hc:", &ch, &optarg) == APR_SUCCESS) {
    switch (ch) {
      case 'I':
  ...
  switch (request) {
    ...
    case COMMAND_NONE:
      log_info("\n\
        'devpkg' is a package manager in C. \n \n \
        Usage: devpkg [OPTION] \n \n \
        Configuration: \n \
          -h  Show this help\n \
          -S  Sets up the installation database\n \
          -I  Installs a piece of software from a Git SSH URL\n \
          -F  Fetches some source code for manual building\n \
          -B  Builds fetches source code and installs it, even if already installed\n \
      ");
  }
